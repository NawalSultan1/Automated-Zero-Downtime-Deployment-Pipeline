...
resource "aws_key_pair" "dev_key" {             //here we are telling aws to create a box with the name key_name and in that key_name add this public key given below
...
resource "aws_security_group" "dev-sg" {
  ...
  ingress {
    description = "http for application traffic"
  ...
  tags = {
    Name = "dev-sg" //giving the server a new meaningful name 
  }
}